...
- **"<ACCESS_TOKEN>" 부분에 반환 된 토큰을 꼭 입력해주세요.**
...
```
- 이벤트 등록시 동일한 이벤트가 있을경우 "동일한 이벤트가 이미 존재합니다." 표출(테스트코드에 2를 붙인 이유 : init으로 이미 동일한 이벤트를 등록)
